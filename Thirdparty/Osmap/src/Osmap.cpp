...
namespace ORB_SLAM2
{
	...
	Osmap::Osmap(System &_system) : map(static_cast<OsmapMap &>(*_system.mpMap)),
	...
	void Osmap::mapLoad(string yamlFilename, bool noSetBad, bool pauseThreads)
	{
		...
		if (pauseThreads)
		{
			...
			if (system.local_object_mapper_)
				system.local_object_mapper_->Release(); // Release local mapper just in case it's stopped, because if it is stopped it can't be reset
			...
			system.mpLocalMapper->RequestStop();
			if (system.local_object_mapper_)
				system.local_object_mapper_->RequestStop();
			if (system.mpViewer)
				system.mpViewer->RequestStop();
			while (!system.mpLocalMapper->isStopped())
				usleep(1000);
			if (system.local_object_mapper_)
				while (!system.local_object_mapper_->isStopped())
					usleep(1000);
			if (system.mpViewer)
		...
		if (system.local_object_mapper_)
			LOGV(system.local_object_mapper_->isStopped())
		if (system.mpViewer)
	...
	void Osmap::getMapPointsFromMap()
	{
		...
		std::transform(map.mspMapPoints.begin(), map.mspMapPoints.end(), std::back_inserter(vectorMapPoints), [](MapPoint *pMP) -> OsmapMapPoint *
									 { return static_cast<OsmapMapPoint *>(pMP); });
		sort(vectorMapPoints.begin(), vectorMapPoints.end(), [](const MapPoint *a, const MapPoint *b)
	...
	void Osmap::getMapObjectsFromMap()
	{
		...
		std::transform(map.map_objects_.begin(), map.map_objects_.end(), std::back_inserter(vectorMapObjects), [](MapObject *pMO) -> OsmapMapObject *
									 { return static_cast<OsmapMapObject *>(pMO); });
		sort(vectorMapObjects.begin(), vectorMapObjects.end(), [](const MapObject *a, const MapObject *b)
				 { return a->GetTrack()->GetId() < b->GetTrack()->GetId(); });
	}
	...
	void Osmap::getKeyFramesFromMap()
	{
		...
		std::transform(map.mspKeyFrames.begin(), map.mspKeyFrames.end(), std::back_inserter(vectorKeyFrames), [](KeyFrame *pKF) -> OsmapKeyFrame *
									 { return static_cast<OsmapKeyFrame *>(pKF); });
		sort(vectorKeyFrames.begin(), vectorKeyFrames.end(), [](const KeyFrame *a, const KeyFrame *b)
				 { return a->mnId < b->mnId; });
	}
...
// ORB-SLAM2 needs to have set mvuRight and mvDepth even though they are not used in monocular.  DUMMY_MAP and OS1 don't have these properties.
			...
			for (int i = 0; i < n; i++)
			{
				...
				if (feature.mappoint_id())
					pKF->mvpMapPoints[i] = getMapPoint(feature.mappoint_id());
 ...
 */
	...
	OsmapMapObject::OsmapMapObject(const Ellipsoid &ell) : MapObject(ell, nullptr){};

	OsmapObjectTrack::OsmapObjectTrack(Osmap *osmap) : ObjectTrack(){};
